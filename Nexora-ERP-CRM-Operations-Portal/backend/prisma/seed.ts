...
async function main() {
  ...
  const users = await Promise.all([
    prisma.user.upsert({
      ...
      update: {},
      create: { name: "Admin User", email: "admin@nexora.com", password: await bcrypt.hash("Admin@123", 10), role: "ADMIN" },
    }),
    prisma.user.upsert({
      where: { email: "sales@nexora.com" },
      update: {},
      create: { name: "Sales User", email: "sales@nexora.com", password: await bcrypt.hash("Sales@123", 10), role: "SALES" },
    }),
    prisma.user.upsert({
      where: { email: "warehouse@nexora.com" },
      update: {},
      create: { name: "Warehouse User", email: "warehouse@nexora.com", password: await bcrypt.hash("Warehouse@123", 10), role: "WAREHOUSE" },
    }),
    prisma.user.upsert({
      where: { email: "accounts@nexora.com" },
      update: {},
      create: { name: "Accounts User", email: "accounts@nexora.com", password: await bcrypt.hash("Accounts@123", 10), role: "ACCOUNTS" },
    }),
  ]);
